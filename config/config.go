...
type Osc struct {
	...
	// 通过 pt-osc 或 gh-ost 修改表的时候, 如果表很大, 修改完表后如果立即删除旧表就很容易产生性能问题,
	// 更多讨论见: https://github.com/github/gh-ost/issues/307
	// 该选项用来控制 pt-osc(对应选项 --drop-old-table), gh-ost(对应选项: --ok-to-drop-table) 开启删除
	// 旧表的时候, 超过指定大小则不进行删除. 单位为 M, 计算方式等同 OscMinTableSize.
	OscMaxDropTableSize uint `toml:"osc_max_drop_table_size" json:"osc_max_drop_table_size"`
